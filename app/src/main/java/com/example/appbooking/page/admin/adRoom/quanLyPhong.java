        ...
        ad_dkl2.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (selectedMainCategory.equals("Chưa có bộ lọc")) {
                    query = "SELECT p.ma_phong, p.vi_tri, lp.ma_loai_phong, lp.ten, lp.gia, lp.so_nguoi_toi_da, lp.mo_ta, ct.id, ct.hinh " +
                            "FROM PHONG p " +
        ...
        try {
            List<List<Object>> row = db1.executeQuery(sql);
                for (List<Object> rows : row) {
                    ...
                    LoaiPhong lp = new LoaiPhong(Integer.parseInt(rows.get(2).toString()), rows.get(3).toString(), Integer.parseInt(rows.get(4).toString()), Integer.parseInt(rows.get(5).toString()), rows.get(6).toString()); // ma_loai_phong, ten, gia, so_nguoi_toi_da, mo_ta
    ...
    private void confirmAndDeleteRoomType(detalPhong roomType, int position) {
        ...
        dialogDelete.setPositiveButton("Xóa", (dialog, which) -> {
            ...
            try{
                ...
                db1.updateSQL("DELETE FROM PHONG" +
                        "WHERE ma_phong = " + ldPhong.get(position).getPhong().getMaPhong() + ";");
                Toast.makeText(quanLyPhong.this, "Phòng đã được xóa", Toast.LENGTH_SHORT).show();
            }catch(Exception e){
                Toast.makeText(quanLyPhong.this, "Lỗi khi xóa phòng", Toast.LENGTH_SHORT).show();
            }
        });
